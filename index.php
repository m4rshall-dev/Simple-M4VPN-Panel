...
if(isset($_POST[$form_names['user']], $_POST[$form_names['password']])) {
    if($csrf->check_valid('post')) {
            ...
            if($user===""){
				...
				if($m4->doLogin($user,$password)){
					$m4->redirect('home.php');
				}else{
					$error = "<script>swal('Oh Snap!','Invalid Username/Password','error');</script>";
				}	
            }
	...
	<div id="login-container">
	    <div id="login">
	        ...
	        <form id="login-form" method="post" class="form">
	            ...
	            <div class="form-group">
	                <input type="submit" name="login-btn" class="btn btn-primary btn-block" value="Sign In" />
	            </div>
	        </form>
	    </div>
	    <a href="register.php" id="signup-btn" class="btn btn-lg btn-block">Create an Account</a>
	</div>
